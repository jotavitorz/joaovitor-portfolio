...
export function ContactPreview() {
    ...
    function sendEmail(e: FormEvent){
        ...
        emailjs.send(emailConfig.serviceId, emailConfig.templateId, templateParams, emailConfig.publicKey)
        .then(() => {
    ...
    return (
        <section className="min-h-full w-full bg-slate-900 py-16">
            ...
            <form onSubmit={sendEmail} className=" flex flex-col mx-auto max-w-[680px] my-10 gap-6 p-4" >
                ...
                <input className="cursor-pointer bg-slate-850 text-amber-50 active:bg-slate-800 h-10 rounded-md font-semibold outline-none focus:border-cyan-800 focus:border-2 shadow-lg shadow-cyan-800/50" onClick={() => setButtonMessage("Enviando...")} type="submit" value={buttonMessage} />
            </form>
        </section>
    )
}
